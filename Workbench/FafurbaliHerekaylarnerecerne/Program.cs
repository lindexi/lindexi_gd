...
IShellLinkW shellLink = (IShellLinkW) Marshal.GetObjectForIUnknown(ppv);
IPersistFile persistFile = (IPersistFile) shellLink;

persistFile.Load(linkFile, STGM_READ);
...
unsafe
{
    fixed (char* c = buffer)
    {
        shellLink.GetPath(c, buffer.Length, out data, SLGP.UNCPRIORITY);
    }
}

Console.WriteLine(new string(buffer));

//static string ResolveShortcut(string filename)
//{
...
{
    void GetPath(char* c, int cchMaxPath, [Out] out WIN32_FIND_DATAW pfd, SLGP fFlags);
