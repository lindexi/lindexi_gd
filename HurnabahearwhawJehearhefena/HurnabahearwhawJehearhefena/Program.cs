...
namespace HurnabahearwhawJehearhefena
{
    ...
    public class AddAndRemoveAppDomainShutdownMonitorTest
    {
        ...
        public IEnumerable<List<IAppDomainShutdownListener>> GetNewAppDomainShutdownMonitorArgList()
        {
            var argList = new List<IAppDomainShutdownListener>();
            for (int i = 0; i < 10000; i++)
            {
                argList.Add(new FakeAppDomainShutdownListener());
            }
