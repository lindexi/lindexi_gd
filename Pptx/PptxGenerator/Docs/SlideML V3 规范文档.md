...
- **`StyleFrom` 提升为通用能力**，所有元素均可引用其他元素作为样式基础，减少重复声明。配合 `StyleId` 属性标记样式模板，使引用关系明确
...
- **`StyleFrom` 为通用属性**，所有可视元素（Panel、Rect、TextElement、Image）均可使用，值为源元素的 `StyleId`。解析时先复制源元素的全部属性作为默认值，再用当前元素显式声明的属性覆盖。不复制子元素
- **`StyleId` 为通用属性**，用于标记元素为样式模板源。带 `StyleId` 的元素可通过 `StyleFrom` 被其他元素引用。`StyleId` 全局唯一，且与 `Id` 独立——同一个元素可以同时有 `Id`（用于布局/流式 Merge）和 `StyleId`（用于被样式引用）
...
| `StyleFrom` | 字符串 | 否 | 引用源元素的 `StyleId`，复制其全部属性作为默认值。不复制子元素 |
| `StyleId` | 字符串 | 否 | 样式模板标识符，全局唯一。带 `StyleId` 的元素可被其他元素通过 `StyleFrom` 引用。悬空元素（不在 Page 子树内的顶层元素）**必须**声明 `StyleId` |
...
> **通用属性**：`Id`、`StyleFrom`、`StyleId`、`X`、`Y`、`Width`、`Height`、`HorizontalAlignment`、`VerticalAlignment`、`Opacity`、`Margin` 见 [§1 通用属性](#1-基本约定)。`Width`/`Height` 不写则自动撑开到包裹所有子元素。流式布局中排列轴方向的 `X`/`Y` 被忽略。流式布局中实际间距 = `max(Gap, 前元素下Margin + 后元素上Margin)`。
...
> **通用属性**：`Id`、`StyleFrom`、`StyleId`、`X`、`Y`、`Width`、`Height`、`HorizontalAlignment`、`VerticalAlignment`、`Opacity`、`Margin` 见 [§1 通用属性](#1-基本约定)。
...
> **通用属性**：`Id`、`StyleFrom`、`StyleId`、`X`、`Y`、`Width`、`Height`、`HorizontalAlignment`、`VerticalAlignment`、`Opacity`、`Margin` 见 [§1 通用属性](#1-基本约定)。`Width` 不写则单行无限宽；`Width` 写了则文本在约束宽度内自动换行。
...
### §12 StyleFrom 与 StyleId 详解

`StyleFrom` 是通用属性（§1 已定义），在流式场景下尤为重要——模型一次定义模板元素，后续所有同类元素通过 `StyleFrom` 引用，只写差异属性。

`StyleFrom` 引用的是源元素的 **`StyleId`**（不是 `Id`）。源元素通过声明 `StyleId` 属性标记自己为样式模板源。

```xml
<!-- 悬空模板（见 §13），带 StyleId -->
<Rect Id="card-template" StyleId="card-style" Width="340" Height="160" Fill="#FFFFFF"
      CornerRadius="12" Shadow="0 4 12 #00000033"
      Stroke="#E8E8E8" StrokeThickness="1"/>

<!-- card2-bg 以 card-style 为样式基础，只声明差异属性 -->
<Rect Id="card2-bg" StyleFrom="card-style" Stroke="#4A7BF7" StrokeThickness="2"/>
<!-- 结果：Width/Height/Fill/CornerRadius/Shadow 照抄源元素，
...
如果 `StyleFrom` 引用的 `StyleId` 不存在，报 `[Error]`，该元素恢复为无 `StyleFrom` 处理。

**`StyleId` 与 `Id` 的关系**：

- `Id` 用于元素在布局树中的标识、流式 Merge 匹配和 Remove 操作
- `StyleId` 用于被 `StyleFrom` 引用作为样式模板源
- 同一个元素可以同时声明 `Id` 和 `StyleId`
- `StyleId` 全局唯一，与 `Id` 互不干扰——一个元素的 `StyleId` 可以与另一个元素的 `Id` 同名
- Page 子树内的元素也可以声明 `StyleId`，被其他元素通过 `StyleFrom` 引用
...
**悬空元素必须声明 `StyleId` 属性**。缺少 `StyleId` 的悬空元素报 `[Error]` 并中断流——这一约束确保 LLM 明确知道哪些元素是样式模板而非页面内容，避免内容丢失。

```xml
<!-- 悬空模板：带 StyleId，不渲染，只作为样式源 -->
<Rect Id="card-template" StyleId="card-style" Width="340" Height="160" Fill="#FFFFFF" CornerRadius="12"/>

<Page Id="main" Background="#F5F5F5">
  <Panel Id="cards" Layout="Horizontal" Gap="24" X="80" Y="100"/>
</Page>

<!-- card1-bg 引用悬空模板的 StyleId -->
<Panel Id="cards">
  <Rect Id="card1-bg" StyleFrom="card-style"/>
</Panel>
```

悬空元素一旦创建即为纯模板，**不可**在后续片段中纳入容器子树。`Id` 全局唯一规则已覆盖此约束——同一个 `Id` 不能出现在两个不同父容器下，而悬空元素无父容器，纳入容器子树将导致 `Id` 冲突。

**设计原因**：LLM 输出一个悬挂的 Style 后，可能忘记这个 Style 并不在 Page 树中，导致它以为相关内容已经被加入页面。引入 `StyleId` 后，使用关系变得明确：只有带 `StyleId` 的元素才允许悬挂，`StyleFrom` 只能引用 `StyleId`。
...
| `StyleFrom` 的源元素 `StyleId` 不存在 | 报 `[Error]`，该元素恢复为无 `StyleFrom` 处理 |
...
| 属性 | 适用标签 | 说明 |
|------|----------|------|
| `RenderSize` | 全部 | 渲染后的实际尺寸，格式 `"宽x高"`，保留两位小数（如 `"340x260"`） |
| `RenderLocation` | 全部（除 Page） | 渲染后的实际布局位置（相对于父容器内容区），格式 `"XxY"`，保留两位小数（如 `"80x400"`） |
...
原始文档加上 `RenderSize`、`RenderLocation`、`ActualLineCount`，模型可以直接对比预期与实际的差异。
...
|------|----------|------|
| `Id` | 全部 | 必填，全局唯一标识符 |
| `StyleFrom` | Panel, Rect, TextElement, Image | 引用源元素 `StyleId`，复制属性作为默认值 |
| `StyleId` | Panel, Rect, TextElement, Image | 样式模板标识符，全局唯一。悬空元素必填 |
