...
- 片段中**未声明的属性** → 保留原有值
- 片段中**未声明的子元素** → 保留原有子元素
- 片段中**显式声明为空字符串的可选属性** → 清除已有值，使该属性恢复为未设置状态
...
由于流式合并中“未声明属性”表示保留旧值，后续片段如果需要让某个可选属性恢复为未设置状态，应将该属性显式设置为空字符串 `""`。解析器应把这种写法解释为**清除该属性**，而不是把空字符串作为普通属性值保存。

```xml
<!-- 初始：标题设置了固定宽度，会在约束宽度内自动换行 -->
<TextElement Id="title" X="80" Y="48" Width="420" Text="SlideML 流式输出" FontSize="36"/>

<!-- 后续：清除 Width，让标题恢复为宽度自适应的单行排版 -->
<TextElement Id="title" Width=""/>

<!-- 结果 -->
<TextElement Id="title" X="80" Y="48" Text="SlideML 流式输出" FontSize="36"/>
```

该规则适用于可选属性，典型场景包括：

- `Width=""`：清除固定宽度，使文本或元素恢复宽度自适应。
- `Height=""`：清除固定高度，使容器或元素恢复高度自适应。
- `X=""` / `Y=""`：清除绝对坐标，使 `HorizontalAlignment` / `VerticalAlignment` 等对齐属性重新生效。
- `Fill=""` / `Stroke=""`：清除显式填充或描边，使其恢复默认值。

空字符串清除规则不适用于必填属性或内容属性。比如 `Text=""` 表示文本内容为空，不表示删除 `Text` 属性；`Id=""`、`Source=""` 等仍应按对应属性的校验规则处理。

不建议使用 `NaN` 表示属性清除。`NaN` 只适合数值属性，无法统一表达颜色、枚举、资源等属性的清除语义，也会把 XML 表达层与内部布局实现细节耦合。
