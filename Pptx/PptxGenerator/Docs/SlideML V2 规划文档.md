...
### 12. Avalonia 渲染引擎 ⚠️ 部分完成

Avalonia 版渲染引擎（`AvaloniaSlideRenderEngine.cs`）已实现渐变画刷和 IsBold/IsItalic，但以下 V2 功能尚未实现：

| 功能 | 状态 |
|------|------|
| 渐变画刷 | ✅ 已实现 |
| IsBold / IsItalic | ✅ 已实现 |
| CornerRadius | ❌ DrawRect 未处理 |
| Shadow 阴影 | ❌ DrawRect 未处理 |
| StrokeDashArray 虚线 | ❌ DrawRect 未处理 |
| Span 富文本 | ❌ DrawText 未处理 |
...
| Avalonia 渲染引擎补全 | 阶段三 | 中 | DrawRect 缺 CornerRadius/Shadow/StrokeDashArray，DrawText 缺 Span 富文本 |
