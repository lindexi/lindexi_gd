...
V2 在 V1 基础上新增了流式布局、渐变填充、阴影、IsBold/IsItalic 字体控制、富文本 Span、圆角、虚线描边等能力，并新增字号等级机制，用于提升多页面课件的文字层级一致性。
...
<Panel Layout="Vertical" Gap="16" Padding="24">
  <TextElement Text="标题" FontSize="L2" IsBold="True" />
  <TextElement Text="正文内容..." FontSize="L3" />
</Panel>
```
...
| `X`, `Y` | 数字 | 否 | 绝对定位模式下相对于父容器的坐标，默认 `0, 0`。流式布局中排列轴方向的坐标若设置则该元素脱离流式布局（见下文） |
...
- 子元素在排列轴方向上的 X 或 Y 若**不设置**，则沿主轴依次排列，由引擎自动计算位置
- 子元素在排列轴方向上的 X 或 Y 若**设置了**，则该元素**脱离流式布局**，使用显式坐标定位（相对于 Panel 内容区原点），不占用流式空间，不影响其他流式元素的位置，也不参与 Panel 自动尺寸计算
- 跨轴方向仍使用声明的坐标或 `VerticalAlignment`/`HorizontalAlignment`
- 子元素超出 Panel 尺寸时产生 Warning，不崩溃

```xml
<!-- 脱离流示例：r2 设置了 X，脱离水平流，不影响 r1 和 r3 的位置 -->
<Panel Layout="Horizontal" Gap="8" Padding="16">
  <Rect Id="r1" Width="100" Height="50" />           <!-- 流式：X=16 -->
  <Rect Id="r2" X="300" Width="80" Height="50" />     <!-- 脱离流：X=300+16=316 -->
...
```xml
<!-- 左上=12, 右上=0, 右下=12, 左下=0 -->
<Rect Width="200" Height="60" Fill="#FFFFFF" CornerRadius="12,0,12,0" />
```
...
| `CornerRadius` | 数字或逗号分隔值 | 否 | 圆角半径，支持 1~4 个值（四角简写规则）：单值 `8` 四角统一；两值 `8,16` 对角相同；三值 `8,16,8`；四值 `8,16,12,20` 分别对应左上、右上、右下、左下。默认 `0` |
...
| 场景 | 示例 |
|------|------|
| 未知字号等级 | `[Error] title: 未知 FontSize 等级 "L6"，已回退到默认字号` |
...
2. **流式布局**：Panel 设置 `Layout="Horizontal"` 或 `Layout="Vertical"` 时，子元素沿排列轴依次排列。若子元素在排列轴上设置了显式 `X`/`Y`，则该元素脱离流式布局，使用绝对坐标定位，不占用流式空间
...
<Page Background="#F5F5F5">
  ...
  <Panel Id="hero" X="0" Y="0" Width="1280" Height="360">
    ...
    <TextElement Id="hero-title" X="80" Y="120" Width="1120"
                 Text="SlideML V2" FontSize="L1" IsBold="True"
  ...
  <Panel Id="cards-row" Layout="Horizontal" Gap="24" X="80" Y="400" Width="1120" Height="280">
    ...
    <TextElement Id="card3-title" X="24" Y="24" Width="292"
                 Text="富文本" FontSize="L4" IsBold="True" Foreground="#333" />
    <TextElement Id="card3-desc" X="24" Y="72" Width="292">
      <Span Text="支持 Span 子元素" FontSize="L5" Foreground="#666"/>
      <Span Text="在同一文本块内" FontSize="L5" IsBold="True" Foreground="#333"/>
      <Span Text="混排多种样式。" FontSize="L5" Foreground="#666"/>
    </TextElement>
  </Panel>
