...
本格式的结构与一致性校验由 `CoursewareFolderLoader` 在加载目录时完成，不再推迟到主题分析阶段。加载失败时不会启动主题 Agent，也不会创建页面美化工作台。

加载阶段会阻断以下问题：不支持的 `ExportVersion`、页数不一致、`SlideIndex` 不连续或顺序不一致、`SlideId` 重复、非有限正数画布、路径越界、Markdown 缺失或未以 `## 页面信息` 开始、Markdown 中 Id/页码/尺寸与清单不一致、资源索引结构错误、资源 ID 重复或资源类型非法。截图或资源实体文件缺失只产生 Warning。
