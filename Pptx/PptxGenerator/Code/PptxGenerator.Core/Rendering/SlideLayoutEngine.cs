...
/// 仅使用 <see cref="SlideRect"/> 和 <see cref="SlidePoint"/> 等纯数据类型。
/// </summary>
public sealed class SlideLayoutEngine : ISlideLayoutEngine
{
    ...
    public void PreLayout(SlidePage page, SlidePipelineContext context)
    {
        ...
        page.LayoutBounds = new SlideRect(0, 0, context.CanvasWidth, context.CanvasHeight);
        ...
        SlideElementMeasurements? measurements)
    {
        ...
        var provisionalContentBounds = new SlideRect(initialChildOriginX, initialChildOriginY, provisionalWidth, provisionalHeight);
        ...
        panel.LocalBounds = new SlideRect(0, 0, actualWidth, actualHeight);
        panel.LayoutBounds = new SlideRect(originX, originY, actualWidth, actualHeight);
        panel.ActualWidth = actualWidth;
        panel.ActualHeight = actualHeight;

        var finalContentBounds = new SlideRect(originX + panel.Padding, originY + panel.Padding, Math.Max(0, actualWidth - panel.Padding * 2), Math.Max(0, actualHeight - panel.Padding * 2));
        ...
        SlideElementMeasurements? measurements)
    {
        ...
        for (var i = 0; i < childSizes.Count; i++)
        {
            ...
            if (isHorizontal)
            {
                child.LocalBounds = new SlideRect(child.X ?? 0, child.Y ?? 0, childWidth, childHeight);
                child.LayoutBounds = new SlideRect(flowPosition, crossOrigin, childWidth, childHeight);
            }
            else
            {
                child.LocalBounds = new SlideRect(child.X ?? 0, child.Y ?? 0, childWidth, childHeight);
                child.LayoutBounds = new SlideRect(crossOrigin, flowPosition, childWidth, childHeight);
            }
        ...
        {
            child.LayoutBounds = new SlideRect(
        ...
        SlidePipelineContext context)
    {
        ...
        rect.LocalBounds = new SlideRect(rect.X ?? 0, rect.Y ?? 0, width, height);
        rect.LayoutBounds = new SlideRect(originX, originY, width, height);
        ...
        SlideElementMeasurements? measurements)
    {
        ...
        text.LocalBounds = new SlideRect(text.X ?? 0, text.Y ?? 0, measuredWidth, measuredHeight);
        text.LayoutBounds = new SlideRect(originX, originY, measuredWidth, measuredHeight);
        ...
        SlideElementMeasurements? measurements)
    {
        ...
        image.LocalBounds = new SlideRect(image.X ?? 0, image.Y ?? 0, width, height);
        image.LayoutBounds = new SlideRect(originX, originY, width, height);
    ...
    private static void ValidateBounds(SlideElement element, SlideRect parentBounds, string parentId, bool clipToParent, SlidePipelineContext context)
    {
    ...
    /// </summary>
    internal static bool SlideRectContains(SlideRect container, SlideRect inner)
    {
