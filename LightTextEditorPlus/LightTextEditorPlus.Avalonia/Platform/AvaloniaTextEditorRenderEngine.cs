...
partial class TextEditor
{
    ...
    /// 额外地，在 Avalonia 存在渲染时机不对齐问题，即可能存在连续两次文本布局渲染，但 Avalonia 只执行一次。这就意味着首次的布局渲染内容没有进入到 <see cref="TextEditorCustomDrawOperation"/> 里面，无法被 <see cref="TextEditorCustomDrawOperation"/> 通过引用计数的方式释放。为了解决此问题，在 ITextEditorContentSkiaRender 添加 IsUsed 属性，通过这个属性判断是否渲染内容被交给 UI 框架了，一旦交给 UI 框架了，那就应该交给 UI 框架释放，不能在 <see cref="SkiaTextEditor"/> 里面释放。如果交给了 UI 框架，在 <see cref="SkiaTextEditor"/> 里面只标记 <see cref="ITextEditorContentSkiaRenderer.IsObsoleted"/> 属性，通过此属性让 <see cref="TextEditorCustomDrawOperation"/> 通过引用计数的方式释放。如果没有交给 UI 框架，那就在 <see cref="SkiaTextEditor"/> 里面释放。
    /// 机制：
...
#if DEBUG
                _debugAllContentSkiaRenderList ??= new List<ITextEditorContentSkiaRenderer>();
                _debugAllContentSkiaRenderList.Add(textEditorSkiaRenderer);
#endif
            }
...
file class TextEditorCustomDrawOperation : ICustomDrawOperation
{
    ...
    public void Dispose()
    {
        ...
        {
            foreach (ITextEditorContentSkiaRenderer textEditorContentSkiaRender in list)
            {
