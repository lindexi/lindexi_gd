    ...
    public static CharHandwritingPaperInfo GetHandwritingPaperInfo(this RenderInfoProvider renderInfoProvider, in ParagraphLineRenderInfo paragraphLineRenderInfo, CharData? charData = null)
    {
        ...
        if (renderInfoProvider.TextEditor.IsInDebugMode)
        {
            if (charData is not null)
            {
                ...
                {
                    throw new InvalidOperationException(ExceptionMessages.RenderManagerExtension_CharDataMustBelongToCurrentLine);
                }
            }
            else
            {
                ...
                foreach (TextReadOnlyListSpan<CharData> _ in lineDrawingArgument.CharList.GetCharSpanContinuous())
                {
                    ...
                    if (count > 1)
                    {
                        throw new ArgumentException(ExceptionMessages.RenderManagerExtension_MustSpecifyCharDataWhenMultipleRunPropertiesExist, nameof(charData));
                    }
                }
            }
        }
        ...
        // 开始的高度应该是计算行高减去字高的距离
        TextSize lineSize = lineDrawingArgument.LineContentSize;
