...
退格删除（Backspace），删除光标前一个字符，如有选中范围则删除选中内容：
...
删除（Delete），删除光标后一个字符，如有选中范围则删除选中内容：

```csharp
TextEditor textEditor = ...
textEditor.AppendText("abc");
// 将光标移动到第一个字符之后
textEditor.CurrentCaretOffset = new CaretOffset(1);
textEditor.Delete();
```

---

按词向前删除（DeleteForwardWord），通常对应 Ctrl+Backspace 快捷键。如有选中内容则删除选中内容，否则从光标位置向前删除到单词开头：

```csharp
TextEditor textEditor = ...
textEditor.AppendText("abc def");
// 光标在 "abc" 末尾：abc| def
textEditor.CurrentCaretOffset = new CaretOffset("abc".Length);
// 删除 "abc" 单词
textEditor.DeleteForwardWord();
```

---

按词向后删除（DeleteBackwardWord），通常对应 Ctrl+Delete 快捷键。如有选中内容则删除选中内容，否则从光标位置向后删除到单词末尾：
...
文本库内部使用两套索引体系：UTF-16 索引（即 `string` 下标）和文档字符偏移（`DocumentOffset`/`Selection` 使用）。当对接外部解析库（如 Markdig、ColorCode）时，外部库返回的字符位置是 UTF-16 索引，不能直接传给 `DocumentOffset` 或 `Selection`，需要使用转换方法：

```csharp
TextEditor textEditor = ...
// 假设 markdownText 是原始 markdown 文本，innerCodeSpan.Start 是外部解析库返回的 UTF-16 索引
string markdownText = "```csharp\ncode\n```";
int utf16Index = 0; // 外部库返回的 UTF-16 索引

// 方法一：直接使用 TextEditor 实例方法（推荐）
DocumentOffset offset1 = textEditor.CreateDocumentOffsetFromUtf16Index(markdownText, utf16Index);

// 方法二：使用 DocumentOffset 静态工厂方法
DocumentOffset offset2 = DocumentOffset.FromUtf16Index(markdownText, utf16Index);

// 方法三：使用 TextIndexConverter 工具类（仅依赖 Core 层时使用）
int docOffset = TextIndexConverter.ConvertUtf16IndexToDocumentOffset(markdownText, utf16Index);

// 反向转换：文档偏移 → UTF-16 索引
int utf16Result = TextIndexConverter.ConvertDocumentOffsetToUtf16Index(markdownText, offset1);
```

以上三种方法均自动处理代理对字符（如 emoji）和 `\r\n` 折叠。如果需要避免 `string` 分配，可使用 `ReadOnlySpan<char>` 重载：

```csharp
ReadOnlySpan<char> textSpan = markdownText.AsSpan();
DocumentOffset offset = textEditor.CreateDocumentOffsetFromUtf16Index(textSpan, utf16Index);
```
...
```csharp
TextEditor textEditor = ...;
GuidingLayoutInfo guidingLayoutInfo = textEditor.GetCurrentGuidingLayoutInfo();
```

指导布局信息可用于获取当前文档被排成了几段、几行，以及每行包含多少个 `CharData` 字符。

如需要让当前文本优先按照某份指导布局进行排版，可设置指导布局：

```csharp
TextEditor textEditor = ...;
GuidingLayoutInfo guidingLayoutInfo = ...;

bool isSuccess = textEditor.SetGuidingLayoutInfoForNextUpdateLayout(guidingLayoutInfo);
```

如果给定的指导布局与当前文本框状态不匹配，例如段落数量、字符数量、布局方式或文本框尺寸不一致，则不会应用，并通过文本日志输出原因。

如需恢复默认布局，可清除指导布局：

```csharp
TextEditor textEditor = ...;
textEditor.ClearGuidingLayoutInfo();
```
...
`TextFeatures` 用于控制公开 API 的可用能力。禁用后，对应 API 会直接返回，不会修改文本内容或样式。
...
// 启用功能
textEditor.EnableFeatures(TextFeatures.SetFontSize);

// 查询某个功能是否启用
bool canSetFontSize = textEditor.IsFeaturesEnable(TextFeatures.SetFontSize);
```

---

示例：禁用字号设置后，`SetFontSize` 不生效：
...
textEditor.SetFontSize(40, textEditor.GetAllDocumentSelection());

// 读取字符属性，字号保持原值
var runProperty = textEditor.GetRunPropertyRange(textEditor.GetAllDocumentSelection()).First();
```

---

示例：禁用段前间距设置后，段落属性不会被修改：

```csharp
TextEditor textEditor = ...
textEditor.Text = "abc";

ParagraphProperty oldProperty = textEditor.GetCurrentCaretOffsetParagraphProperty();
textEditor.DisableFeatures(TextFeatures.SetParagraphSpaceBefore);

ParagraphProperty newProperty = oldProperty with
{
    ParagraphBefore = oldProperty.ParagraphBefore + 20
};
textEditor.SetCurrentCaretOffsetParagraphProperty(newProperty);

ParagraphProperty currentProperty = textEditor.GetCurrentCaretOffsetParagraphProperty();
```
