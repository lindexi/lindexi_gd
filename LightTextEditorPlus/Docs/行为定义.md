...
默认采用按词换行策略，即如果单行空间足够放下单词，单词不拆分为两行布局，否则宁可留空也不拆词
...
中文默认不做分词。但很多正式出版物都有维护专用词，避免被断行分开，排版过程中正常都是依靠更改字间距和标点符号宽度让专用词不被分割。如 [The Type — 行行当机不立断](https://www.thetype.com/2018/01/14017/ ) 所述：

> 事实上，在实际的媒体工作中，中国有很多报社都需要「专用词管理」，定义一些需要「特殊关照」的词汇并进行维护，确保它们在换行时不会被断开。这些所谓的「专用词」，基本上是重要的专有名词，可以是人名，也可以是国名、地名，而具体的定义则没有固定标准，依照报社、出版社的特例风格和规定而有所不同
...
- 空段落对外至少表现为一段一行
- 多段和多行按照实际布局结果输出
- 对齐影响行内容起始点，但不改变行内字符顺序
- 横排、竖排和蒙文竖排都应输出统一结构的行段结果
- 当设置了有效的指导布局时，布局应优先遵循指导布局的每行字符数
- 当指导布局与当前文本状态不匹配时，应回退默认布局而不是强行套用
