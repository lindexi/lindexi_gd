...
也就是即使输入的是 `1\n23` 字符串，获取字符数量，将会是 4 个字符；输入的是 `1\r\n23` 字符串，获取字符数量也依然会是 4 个字符，且获取到为 `1\n23` 字符串
...
也如上图所示，可见获取光标对应的字符是没有固定的方法的。虽然绝大部分情况下，取光标的序号减一对应的字符就是满足需求的，但仍然会有一些业务上的例外项，还请根据实际需求决定获取字符的方式
...
指导布局也称指导渲染。由于 `LightTextEditorPlus.Core` 不直接负责具体渲染，因此在核心层统一称为“指导布局”。

指导布局表示一份平台无关的布局描述信息，既可以由当前布局结果导出，也可以反向设置给文本库，用来指导后续布局过程。
...
也称 整词换行。为排版中的策略： 宁可留空也不拆词

对应着尽量塞满（允许拆词）的策略

整词换行需要有分词策略才能正确进行。简单做法是按照空格分行。但这在泰语、老挝语和柬埔寨语文化下不成立，在这些语言中，书写时单词之间不包含空格。其分词中，如果人类有明确输入 Unicode 字符 U+200B ZERO WIDTH SPACE （ZWSP）字符，则照此分词。否则需要有语言文化协助分词

如果允许拆词，则在语言文化协助下，可进行音节边界进行换行。额外地： 在中文公文编写中，存在“领导名字不折行”的规则。正常来说遇到此规则，还需要配合字间距控制，从而达到行不留空且名称不折行的效果。详细请参阅 [行为定义](./行为定义.md)

参考文档： [断行的方法](https://www.w3.org/International/articles/typography/linebreak ) 和 [The Type — 文字 / 设计 / 文化 — 行行当机不立断](https://www.thetype.com/2018/01/14017/ )