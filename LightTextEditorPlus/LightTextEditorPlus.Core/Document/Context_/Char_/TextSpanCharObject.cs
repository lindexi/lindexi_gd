...
public sealed class TextSpanCharObject : ICharObject, IEquatable<string>
{
    ...
    public TextSpanCharObject(string originText, int charIndex, int charCount = 1)
    {
        ...
        else if (charCount == 2)
        {
            ...
            {
                throw new ArgumentException(ExceptionMessages.TextSpanCharObject_CharCountMustBeSurrogatePair, nameof(charCount));
            }
        }
        else
        {
            // 错误情况
            throw new ArgumentException(string.Format(System.Globalization.CultureInfo.CurrentCulture, ExceptionMessages.TextSpanCharObject_CharCountMustBeOneOrTwo, charCount),
                nameof(charCount));
        }
    }
