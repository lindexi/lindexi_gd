...
小希输入法进程内 IME 模块。职责：使用 C# NativeAOT 直接发布为 `XiaoXiIme.ime`、导出 IME ABI 入口、注册 IME UI 窗口类、接收系统调用、通过 IPC 转发输入事件并写回必要 IME 上下文。

当前已具备最小 IMM 转换消息骨架：

- `ImeExports` 直接提供非托管导出入口，具体逻辑转发到可测试的托管运行时。
- `ImeUiWindowClass` 使用 P/Invoke 在 C# 中注册 `CS_IME` UI 窗口类，不依赖 C++ 外壳。
