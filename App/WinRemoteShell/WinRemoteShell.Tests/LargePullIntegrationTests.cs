...
[TestClass]
[DoNotParallelize]
public sealed class LargePullIntegrationTests
{
    ...
    public async Task WhenDirectoryLargerThanOneGiBIsPulledThenEntireTreeIsPreserved()
    {
        var root = Path.Join(AppContext.BaseDirectory, $"WinRemoteShell_LargePull_{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            {
                await PullClient.PullAsync(host.Address, source, output);
            }

            var outputManifest = await CreateManifestAsync(output);

            CollectionAssert.AreEqual(sourceManifest, outputManifest);
        }
        finally
        {
    ...
    private static async Task CreateSourceTreeAsync(string source)
    {
        ...
        await WriteRandomFileAsync(Path.Join(source, "root-large.bin"), 640L * 1024 * 1024, 1701);
        await WriteRandomFileAsync(Path.Join(firstLevel, "nested-large.bin"), 384L * 1024 * 1024, 1702);
        await WriteRandomFileAsync(Path.Join(thirdLevel, "deep file.bin"), 64L * 1024 * 1024, 1703);

        var totalLength = Directory.EnumerateFiles(source, "*", SearchOption.AllDirectories)
            .Sum(path => new FileInfo(path).Length);
        if (totalLength <= OneGibibyte)
        {
            throw new InvalidOperationException($"Large pull test data must exceed one GiB, but was {totalLength} bytes.");
        }
    }
