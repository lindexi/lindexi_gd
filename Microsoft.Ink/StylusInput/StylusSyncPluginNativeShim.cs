...
// Type: Microsoft.StylusInput.StylusSyncPluginNativeShim
...
namespace Microsoft.StylusInput
{
  internal sealed class StylusSyncPluginNativeShim : StylusSyncPluginNative, IStylusPluginNative
  {
    ...
    public void CursorInRange(IRealTimeStylusNative realTime, uint tcid, uint cid)
    {
      ...
      {
        if (this.m_first)
          this.m_RealTimeStylus.AddCursorToCollection((int) cid, (int) tcid);
        if (this.m_Sink == null)
          return;
        this.m_Sink.StylusInRange(this.m_RealTimeStylus, new StylusInRangeData(this.m_RealTimeStylus.GetStylusForId((int) cid)));
      }
      catch (Exception ex)
      {
      ...
      ref IntPtr InOutPkt)
    {
      try
      {
        ...
        uint cPkts = 1;
        StylusSyncPluginNativeShim.MarshalQueueDataIfChanged((StylusDataBase) data, ref InOutPkt, ref cPkts);
      }
      catch (Exception ex)
      {
      ...
      ref IntPtr InOutPkts)
    {
      try
      {
        ...
        StylusSyncPluginNativeShim.MarshalQueueDataIfChanged((StylusDataBase) data, ref InOutPkts, ref cInOutPkts);
      }
      catch (Exception ex)
      {
