...
using JajurjabokarruWholokarka.Diagnostics;
using System;
...
namespace JajurjabokarruWholokarka;
...
class DemoWindow
{
    ...
    private unsafe HWND CreateWindow()
    {
        ...
        fixed (char* pTitle = title)
        {
            ...
            {
                Console.Read();
                Console.WriteLine("Set Window long");

                var windowLong = GetWindowLong(windowHwnd, WINDOW_LONG_PTR_INDEX.GWL_EXSTYLE);
                var exists = (windowLong | (int) WINDOW_EX_STYLE.WS_EX_NOREDIRECTIONBITMAP) != 0;

                if (exists)
                {
                    // 去掉 WS_EX_NOREDIRECTIONBITMAP 样式
                    windowLong &= ~(int) WINDOW_EX_STYLE.WS_EX_NOREDIRECTIONBITMAP;
                    var success = SetWindowLong(windowHwnd, WINDOW_LONG_PTR_INDEX.GWL_EXSTYLE, windowLong);

                    if (success == 0)
                    {
                        // 失败了
                        var error = Marshal.GetLastWin32Error();
                        Console.WriteLine($"SetWindowLong 失败，错误码 {new Win32Exception(error).ToString()}");
                    }
                }
            });
