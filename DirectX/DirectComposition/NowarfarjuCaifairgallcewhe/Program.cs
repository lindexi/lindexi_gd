...
using NowarfarjuCaifairgallcewhe.Diagnostics;
using System;
...
namespace NowarfarjuCaifairgallcewhe;
    ...
    private void RenderCore()
    {
        ...
        while (!_isDisposed)
        {
            if (_isReSize)
            {
                ...
                var surface = _renderContext.Surface;
                surface.Resize((uint) (clientSize.Width),
            ...
            {
                var surface = _renderContext.Surface;
                using var d3D11Texture2D = surface.BeginDraw<ID3D11Texture2D>(null, out var updateOffset);
                _ = updateOffset;
                ...
                _renderContext.CompositionDevice?.WaitForCommitCompletion();
            }
        }
    }
