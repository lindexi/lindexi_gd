...
另外，`ICommandPattern` 现在带有 `Priority` 属性，`CommandPatternManager` 会在注册时按优先级降序插入，同优先级保持原始注册顺序。

这意味着：

- 越重要、越常用的命令应给更高优先级；
- 与模型相关的命令可以稳定出现在菜单更靠前的位置；
- 不需要依赖“先注册就靠前”的隐式约定。
...
“润色选中文本”属于模型相关命令，因此放在 `RightSlideBar` 侧的 `CopilotPatternProvider` 注册，并设置较高优先级。

该命令有 4 个关键约束：

1. 只在存在真实选区时显示，因此命令模式需要 `SupportSingleLine = false`。
2. 发起请求前先记录选区范围、原始文本以及前后文片段，作为后续安全替换依据。
3. 模型处理不直接读取聊天输出，而是要求 Agent 必须调用工具，把“润色后的完整文本”作为工具参数提交。
4. 大模型完成后，不立即盲目替换，而是先在当前文档中重新定位原始选中文本；只有确认原文本仍未变化时才替换，否则在侧边栏说明不替换原因。

当前的范围适配策略不是维护实时可变锚点，而是：

- 优先尝试原始偏移量直接命中；
- 如果原位置已偏移，则在当前全文里搜索原始文本；
- 结合选区前后文和距离原始偏移量的远近，找出最可信的位置；
- 如果文本已变化，或者出现多处同分候选导致无法安全判定，就放弃替换。

这样做的目标是“宁可不替换，也不要误替换”。
