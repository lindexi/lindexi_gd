...
因此，编辑器右键发送与用户手动在侧栏输入，其实最终都进入同一个 `CopilotViewModel` 会话状态。

这也是“为什么发送选中内容后，消息会直接出现在右侧 Copilot 聊天历史里”的根本原因。
...
| `RightSlideBar` / `CopilotHandler` | 将主 ViewModel 与共享聊天能力接通 | 不处理编辑器选区细节 |
| `CopilotViewModel` | 管理会话状态、消息集合和模型调用 | 不知道消息来自编辑器右键还是输入框 |
...
- 聊天状态是共享的，编辑器发送与侧栏手输复用同一份 `ChatMessages`。
