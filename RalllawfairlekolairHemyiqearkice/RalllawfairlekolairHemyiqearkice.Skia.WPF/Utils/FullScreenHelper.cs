...
namespace RalllawfairlekolairHemyiqearkice.Skia.Wpf.Utils
{
    ...
    /// </summary>
    /// 参考 [WPF 稳定的全屏化窗口方法](https://blog.lindexi.com/post/WPF-%E7%A8%B3%E5%AE%9A%E7%9A%84%E5%85%A8%E5%B1%8F%E5%8C%96%E7%AA%97%E5%8F%A3%E6%96%B9%E6%B3%95.html )
    public static partial class FullScreenHelper
    {
        ...
        private static readonly DependencyProperty BeforeFullScreenWindowPlacementProperty =
            DependencyProperty.RegisterAttached("BeforeFullScreenWindowPlacement", typeof(FullScreenHelper.WINDOWPLACEMENT?),
                typeof(Window));
        ...
        private static readonly DependencyProperty BeforeFullScreenWindowStyleProperty =
            DependencyProperty.RegisterAttached("BeforeFullScreenWindowStyle", typeof(FullScreenHelper.WindowStyles?), typeof(Window));
        ...
        public static void StartFullScreen(Window window)
        {
                ...
                window.GetValue(BeforeFullScreenWindowStyleProperty) == null)
            {
                ...
                //获取当前窗口的位置大小状态并保存
                var placement = new FullScreenHelper.WINDOWPLACEMENT();
                placement.Size = (uint) Marshal.SizeOf(placement);
                FullScreenHelper.Win32.User32.GetWindowPlacement(hwnd, ref placement);
                window.SetValue(BeforeFullScreenWindowPlacementProperty, placement);

                //修改窗口样式
                var style = (FullScreenHelper.WindowStyles) FullScreenHelper.Win32.User32.GetWindowLongPtr(hwnd, FullScreenHelper.GetWindowLongFields.GWL_STYLE);
                ...
                style &= (~(FullScreenHelper.WindowStyles.WS_THICKFRAME | FullScreenHelper.WindowStyles.WS_MAXIMIZEBOX | FullScreenHelper.WindowStyles.WS_MAXIMIZE | FullScreenHelper.WindowStyles.WS_CAPTION));
                FullScreenHelper.Win32.User32.SetWindowLongPtr(hwnd, FullScreenHelper.GetWindowLongFields.GWL_STYLE, (IntPtr) style);

                //禁用 DWM 过渡动画 忽略返回值，若DWM关闭不做处理
                FullScreenHelper.Win32.Dwmapi.DwmSetWindowAttribute(hwnd, FullScreenHelper.DWMWINDOWATTRIBUTE.DWMWA_TRANSITIONS_FORCEDISABLED, 1,
                    sizeof(int));

                //添加Hook，在窗口尺寸位置等要发生变化时，确保全屏
                hwndSource.AddHook(KeepFullScreenHook);

                if (FullScreenHelper.Win32.User32.GetWindowRect(hwnd, out var rect))
                {
                    //不能用 placement 的坐标，placement是工作区坐标，不是屏幕坐标。

                    //使用窗口当前的矩形调用下设置窗口位置和尺寸的方法，让Hook来进行调整窗口位置和尺寸到全屏模式
                    FullScreenHelper.Win32.User32.SetWindowPos(hwnd, (IntPtr) FullScreenHelper.HwndZOrder.HWND_TOP, rect.Left, rect.Top, rect.Width,
                        rect.Height, (int) FullScreenHelper.WindowPositionFlags.SWP_NOZORDER);
                }
            }
        }
        ...
        public static void EndFullScreen(Window window)
        {
            ...
            //确保在全屏模式并获取之前保存的状态
            if (window.GetValue(BeforeFullScreenWindowPlacementProperty) is FullScreenHelper.WINDOWPLACEMENT placement
                && window.GetValue(BeforeFullScreenWindowStyleProperty) is FullScreenHelper.WindowStyles style)
            {
                ...
                FullScreenHelper.Win32.User32.SetWindowLongPtr(hwnd, FullScreenHelper.GetWindowLongFields.GWL_STYLE,
                    (IntPtr) (style & (~(FullScreenHelper.WindowStyles.WS_MAXIMIZE | FullScreenHelper.WindowStyles.WS_MINIMIZE))));

                if ((style & FullScreenHelper.WindowStyles.WS_MINIMIZE) != 0)
                {
                    //如果窗口进入全屏前是最小化的，这里不让窗口恢复到之前的最小化状态，而是到还原的状态。
                    //大多数情况下，都不期望在退出全屏的时候，恢复到最小化。
                    placement.ShowCmd = FullScreenHelper.Win32.ShowWindowCommands.SW_RESTORE;
                }

                if ((style & FullScreenHelper.WindowStyles.WS_MAXIMIZE) != 0)
                {
                    //提前调用 ShowWindow 使窗口恢复最大化，若通过 SetWindowPlacement 最大化会导致闪烁，只靠其恢复 RestoreBounds.
                    FullScreenHelper.Win32.User32.ShowWindow(hwnd, FullScreenHelper.Win32.ShowWindowCommands.SW_MAXIMIZE);
                }

                FullScreenHelper.Win32.User32.SetWindowPlacement(hwnd, ref placement);

                if ((style & FullScreenHelper.WindowStyles.WS_MAXIMIZE) ==
                ...
                FullScreenHelper.Win32.Dwmapi.DwmSetWindowAttribute(hwnd, FullScreenHelper.DWMWINDOWATTRIBUTE.DWMWA_TRANSITIONS_FORCEDISABLED, 0,
                    sizeof(int));
        ...
        private static IntPtr KeepFullScreenHook(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
        {
            ...
            if (msg == WINDOWPOSCHANGING)
            {
                ...
                {
                    //得到WINDOWPOS结构体
                    var pos = (FullScreenHelper.WindowPosition) Marshal.PtrToStructure(lParam, typeof(FullScreenHelper.WindowPosition));

                    if ((pos.Flags & FullScreenHelper.WindowPositionFlags.SWP_NOMOVE) != 0 &&
                        (pos.Flags & FullScreenHelper.WindowPositionFlags.SWP_NOSIZE) != 0)
                    {
                        //既然你既不改变位置，也不改变尺寸，我就不管了...
                        return IntPtr.Zero;
                    }

                    if (FullScreenHelper.Win32.User32.IsIconic(hwnd))
                    {
                    ...
                    if (FullScreenHelper.Win32.User32.GetWindowRect(hwnd, out var rect))
                    {
                        ...
                        if ((pos.Flags & FullScreenHelper.WindowPositionFlags.SWP_NOSIZE) == 0)
                        {
                            //要改变尺寸
                        ...
                        //使用目标矩形获取显示器信息
                        var monitor = FullScreenHelper.Win32.User32.MonitorFromRect(targetRect, FullScreenHelper.MonitorFlag.MONITOR_DEFAULTTOPRIMARY);
                        var info = new FullScreenHelper.MonitorInfo();
                        info.Size = (uint) Marshal.SizeOf(info);
                        if (FullScreenHelper.Win32.User32.GetMonitorInfo(monitor, ref info))
                        {
                            //基于显示器信息设置窗口尺寸位置
                            ...
                            pos.Flags &= ~(FullScreenHelper.WindowPositionFlags.SWP_NOSIZE | FullScreenHelper.WindowPositionFlags.SWP_NOMOVE |
                                           FullScreenHelper.WindowPositionFlags.SWP_NOREDRAW);
                            pos.Flags |= FullScreenHelper.WindowPositionFlags.SWP_NOCOPYBITS;
