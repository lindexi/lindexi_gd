...
var folder = @"C:\lindexi\Work\古典名著\";

var configurationFile = @"C:\lindexi\Work\Key\AgentConfiguration.json";
var apiManagerConfiguration = await AgentApiManagerConfiguration.FromJsonFileAsync(new FileInfo(configurationFile));
var agentApiEndpointManager = new AgentApiEndpointManager();
agentApiEndpointManager.LoadConfiguration(apiManagerConfiguration);

var languageModel = agentApiEndpointManager.GetModel("deepseek-v4-pro", "CCH");
...
[
    new ChatMessage(ChatRole.System, "请你根据我传入的古籍内容，从中选取出适合做小孩子名字的词。要求不能被轻易起花名，起名应该避开神仙的名字和皇帝溢号。请列出所选的名字的词和选择原因，完成所有选择之后，请调用工具将所选的名字进行整理。如果读取到的文本是繁体中文，请将其转换为简体中文后再进行处理"),
];
