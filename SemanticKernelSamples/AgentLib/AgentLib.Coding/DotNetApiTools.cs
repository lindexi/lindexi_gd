...
internal sealed class DotNetApiTools
{
    ...
    [
        new(AIFunctionFactory.Create(ListDotNetApiAsync, "ListDotNetApi"), arguments => ToolCallPresentationFactory.ForQuery(arguments, "source")),
        new(AIFunctionFactory.Create(GetDotNetTypeApiAsync, "GetDotNetTypeApi"), arguments => ToolCallPresentationFactory.ForQuery(arguments, "typeName")),
    ];
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            IReadOnlyList<ApiType> pageTypes = filteredTypes
                ...
                .Take(PageSize)
                .ToArray();
            return Task.FromResult(FormatTypeList(resolution.DisplayName!, allTypes.Count, filteredTypes.Count, keyword, requestedPage, pageCount, pageTypes));
        }
    ...
    private SourceResolution ResolveSource(string source)
    {
        ...
        string[] assemblyPaths = Directory.Exists(assemblyRoot)
            ? Directory.GetFiles(assemblyRoot, "*.dll", SearchOption.AllDirectories).OrderBy(path => path, StringComparer.OrdinalIgnoreCase).ToArray()
            : [];
    ...
    private static IReadOnlyList<ApiType> ReadPublicTypes(IReadOnlyList<string> assemblyPaths, CancellationToken cancellationToken)
    {
        ...
        foreach (string assemblyPath in assemblyPaths)
        {
            ...
            foreach (TypeDefinitionHandle handle in reader.TypeDefinitions)
            {
                ...
                if (!IsPublicType(reader, handle, definition) || IsCompilerGenerated(reader, definition.GetCustomAttributes()))
                {
                    continue;
                }
                ...
                {
                    types.Add(fullName, new ApiType(fullName, GetTypeKind(reader, definition), documentation.GetSummary("T:" + fullName)));
                }
            }
        }
        ...
        IReadOnlyList<ApiType> types)
    {
        ...
        builder.AppendLine("，");
        builder.Append("当前为第 ").Append(page).Append(" 页，每页 ").Append(PageSize).Append(" 个，共 ").Append(pageCount).AppendLine(" 页。");
    ...
    private static string? TryFormatType(string assemblyPath, string requestedTypeName, CancellationToken cancellationToken)
    {
        using FileStream stream = File.OpenRead(assemblyPath);
        using var peReader = new PEReader(stream);
        if (!peReader.HasMetadata)
        ...
        foreach (TypeDefinitionHandle handle in reader.TypeDefinitions)
        {
            ...
            return FormatType(reader, handle, definition, fullName, XmlDocumentation.Load(Path.ChangeExtension(assemblyPath, ".xml")));
        }
        ...
        XmlDocumentation documentation)
    {
        ...
        AppendSection(builder, "属性", definition.GetProperties().Select(propertyHandle => FormatProperty(reader, propertyHandle, fullName, provider, context, documentation)).Where(value => value is not null)!);
        AppendSection(builder, "字段", definition.GetFields().Select(fieldHandle => FormatField(reader, fieldHandle, fullName, provider, context, documentation)).Where(value => value is not null)!);
        AppendSection(builder, "事件", definition.GetEvents().Select(eventHandle => FormatEvent(reader, eventHandle, fullName, provider, context, documentation)).Where(value => value is not null)!);
        ...
        GenericContext context)
    {
        ...
        string modifiers = (definition.Attributes & TypeAttributes.Abstract) != 0 && (definition.Attributes & TypeAttributes.Sealed) != 0
            ? "static "
            : (definition.Attributes & TypeAttributes.Abstract) != 0 && kind == "类" ? "abstract "
            : (definition.Attributes & TypeAttributes.Sealed) != 0 && kind == "类" ? "sealed " : string.Empty;
        ...
        bases.AddRange(definition.GetInterfaceImplementations().Select(interfaceHandle =>
        {
        ...
        string inheritance = bases.Count == 0 ? string.Empty : " : " + string.Join(", ", bases.Distinct(StringComparer.Ordinal));
        ...
        XmlDocumentation documentation)
    {
        ...
        for (int index = 0; index < signature.ParameterTypes.Length; index++)
        {
            ...
            string prefix = GetParameterPrefix(reader, parameter, parameterType, index == 0 && IsExtensionMethod(reader, method.GetCustomAttributes()));
        ...
        string documentationId = BuildMethodDocumentationId(reader, declaringTypeName, metadataName, signature, context);
        return FormatMemberBlock(reader, declaration, method.GetCustomAttributes(), provider, documentation, documentationId);
    }
        ...
        XmlDocumentation documentation)
    {
        ...
        string accessorText = $"{{{(getter is not null ? " get;" : string.Empty)}{(setter is not null ? " set;" : string.Empty)} }}";
        string modifiers = getter is not null && (getter.Value.Attributes & MethodAttributes.Static) != 0
            || setter is not null && (setter.Value.Attributes & MethodAttributes.Static) != 0 ? "static " : string.Empty;
        string declaration;
        if (signature.ParameterTypes.Length > 0)
        {
            string parameters = string.Join(", ", signature.ParameterTypes.Select((type, index) => $"{ApplyGenericNames(type, context, true)} arg{index}"));
        ...
        return FormatMemberBlock(reader, declaration, property.GetCustomAttributes(), provider, documentation, documentationId);
    }
        ...
        XmlDocumentation documentation)
    {
        ...
        string modifiers = (field.Attributes & FieldAttributes.Literal) != 0 ? "const "
            : (field.Attributes & FieldAttributes.Static) != 0 && (field.Attributes & FieldAttributes.InitOnly) != 0 ? "static readonly "
            : (field.Attributes & FieldAttributes.Static) != 0 ? "static "
            : (field.Attributes & FieldAttributes.InitOnly) != 0 ? "readonly " : string.Empty;
        string value = field.GetDefaultValue().IsNil ? string.Empty : " = " + FormatConstant(reader, reader.GetConstant(field.GetDefaultValue()));
        string declaration = $"public {modifiers}{fieldType} {name}{value}";
        return FormatMemberBlock(reader, declaration, field.GetCustomAttributes(), provider, documentation, "F:" + declaringTypeName + "." + name);
    }
        ...
        XmlDocumentation documentation)
    {
        ...
        string modifiers = adder is not null && (adder.Value.Attributes & MethodAttributes.Static) != 0 ? "static " : string.Empty;
        string declaration = $"public {modifiers}event {eventType} {name}";
        return FormatMemberBlock(reader, declaration, eventDefinition.GetCustomAttributes(), provider, documentation, "E:" + declaringTypeName + "." + name);
    }
        ...
        string documentationId)
    {
        ...
        AppendDocumentation(builder, documentation.GetSummary(documentationId), documentation.GetParameters(documentationId), documentation.GetReturns(documentationId));
    ...
    private static void AppendDocumentation(StringBuilder builder, string? summary, IReadOnlyDictionary<string, string>? parameters, string? returns)
    {
    ...
    private static string GetParameterPrefix(MetadataReader reader, Parameter parameter, string type, bool isExtensionReceiver)
    {
        ...
        GenericContext context)
    {
        ...
        foreach (GenericParameterHandle handle in handles)
        {
            ...
            values.AddRange(parameter.GetConstraints().Select(constraintHandle =>
            {
        ...
        GenericContext context)
    {
        ...
        string parameters = string.Join(",", signature.ParameterTypes.Select(type => ToDocumentationTypeName(ApplyGenericNames(type, context, true))));
    ...
    private static bool IsAttribute(MetadataReader reader, CustomAttributeHandleCollection attributes, string attributeTypeName)
    {
    ...
    private static string? GetObsoleteText(MetadataReader reader, CustomAttributeHandleCollection attributes, CSharpSignatureTypeProvider provider)
    {
        foreach (CustomAttributeHandle handle in attributes)
        {
            ...
            try
            {
                ...
                return value.FixedArguments.Length > 0 ? value.FixedArguments[0].Value as string ?? string.Empty : string.Empty;
            }
            catch (BadImageFormatException)
            {
                return string.Empty;
    ...
    {
        internal static SourceResolution Succeeded(string displayName, IReadOnlyList<string> assemblyPaths) => new(displayName, assemblyPaths, null);
    ...
    private readonly record struct GenericContext(ImmutableArray<string> TypeParameters, ImmutableArray<string> MethodParameters)
    {
        internal static GenericContext ForType(MetadataReader reader, TypeDefinition definition) =>
            new(definition.GetGenericParameters().Select(handle => reader.GetString(reader.GetGenericParameter(handle).Name)).ToImmutableArray(), ImmutableArray<string>.Empty);

        internal static GenericContext ForMethod(MetadataReader reader, MethodDefinition method, ImmutableArray<string> typeParameters) =>
            new(typeParameters, method.GetGenericParameters().Select(handle => reader.GetString(reader.GetGenericParameter(handle).Name)).ToImmutableArray());
    }

    private sealed class CSharpSignatureTypeProvider : ISignatureTypeProvider<string, GenericContext>, ICustomAttributeTypeProvider<string>
    {
        ...
        public string GetTypeFromDefinition(MetadataReader reader, TypeDefinitionHandle handle, byte rawTypeKind) => GetFullTypeName(reader, handle);
        public string GetTypeFromReference(MetadataReader reader, TypeReferenceHandle handle, byte rawTypeKind) => GetFullTypeName(reader, handle);
        public string GetTypeFromSpecification(MetadataReader reader, GenericContext genericContext, TypeSpecificationHandle handle, byte rawTypeKind) => reader.GetTypeSpecification(handle).DecodeSignature(this, genericContext);
        public string GetSystemType() => "System.Type";
        public bool IsSystemType(string type) => type == "System.Type";
        public string GetTypeFromSerializedName(string name) => name;
        public PrimitiveTypeCode GetUnderlyingEnumType(string type) => PrimitiveTypeCode.Int32;

        internal string GetTypeFromHandle(MetadataReader reader, EntityHandle handle, GenericContext context) => handle.Kind switch
        {
