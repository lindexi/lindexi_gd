...
public sealed class ChatWorkspaceViewModel : ViewModelBase
{
    ...
    private async Task SendMessageAsync()
    {
        ...
        try
        {
            ...
            {
                assistantMessage.Message.AppendText("模型未返回正文内容。");
            }
        ...
        catch (OperationCanceledException)
        {
            if (string.IsNullOrWhiteSpace(assistantMessage.Content))
            {
                assistantMessage.Message.AppendText("已停止生成。");
            }
        ...
        {
            assistantMessage.ReplaceContent($"生成失败：{exception.Message}", assistantMessage.ThoughtContent);
            StatusMessage = "生成失败";
